...
export default function TabTwoScreen({ navigation }: any) {
    ...
    const filteredProducts: IProduct[] = products.filter((product: IProduct) => {
        if (filter.includes(product.category) || filter.includes("all")) return product;
    });
        ...
        <View style={styles.container}>
            ...
            {isProductsLoading === "loading" ? (
                ...
                <View style={styles.products}>
                    <Text style={styles.productsLengthText}>{filteredProducts.length} items</Text>
                    <ScrollView>
                        <FlatList
                            numColumns={2}
                            horizontal={false}
                            ...
                            renderItem={({ item }) => (
                                <View key={item.id} style={styles.prdouct}>
                                    <TouchableOpacity
                                        onPress={() =>
                                            navigation.navigate("Product", { title: item.title, price: item.price })
                                        }
                                    >
                                        <Image
                                            alt={item.title}
